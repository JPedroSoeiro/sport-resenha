...
export function PlayerSetupView() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-[#0a0a0f]">
        <div className="text-center max-w-md w-full">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#00ff88]/10 border border-[#00ff88]/30 mb-6">
          ...
          <p className="text-sm text-[#888899] mb-8">
            Insira os nomes dos 3 gerentes que participarão do jogo
          </p>

          <div className="space-y-4 mb-8">
            {[0, 1, 2].map((index) => (
              <div key={index} className="text-left">
                <Label htmlFor={`player-${index}`} className="text-sm text-[#888899] mb-2 block">
                  Gerente {index + 1}
                </Label>
                <div className="relative">
                  <UserPlus className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#888899]" />
                  <Input
                    id={`player-${index}`}
                    type="text"
                    ...
                    className="pl-10 bg-[#12121a] border-[#2a2a38] text-[#f0f0f5] placeholder:text-[#666677]"
                  />
                  {playerNames[index].trim() && (
                    <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#00ff88]" />
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="p-4 rounded-xl bg-[#12121a] border border-[#2a2a38] mb-6">
            <p className="text-xs text-[#888899]">Orçamento por time</p>
            <p className="text-xl font-bold text-[#00ff88]">{formatCurrency(state.globalBudget)}</p>
          </div>

          <Button
            onClick={handleConfirmNames}
            disabled={!allNamesValid}
            className="w-full h-12 font-bold bg-[#00ff88] hover:bg-[#00dd77] text-[#0a0a0f] disabled:opacity-50 disabled:cursor-not-allowed"
          >
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-8 bg-[#0a0a0f]">
      <div className="text-center max-w-4xl w-full">
        ...
        {currentManager && (
          <div className="mb-6">
            <p className="text-[#888899]">Vez de</p>
            <p className="text-xl font-bold text-[#00ff88]">{currentManager.name}</p>
            <p className="text-sm text-[#666677]">Escolha seu time do Brasileirão</p>
          </div>
        )}
            ...
            return (
              <div key={manager.id} className="flex items-center gap-2">
                <div
                  className={cn(
                    ...
                    hasSelected
                      ...
                      : "border-[#2a2a38] bg-[#12121a] text-[#888899]"
                  )}
                >
                ...
                {index < state.managers.length - 1 && (
                  <div className={cn(
                    "w-8 h-0.5",
                    hasSelected ? "bg-[#00ff88]" : "bg-[#2a2a38]"
                  )} />
                )}
              </div>
            )
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected
                    ? "border-[#2a2a38] bg-[#12121a]/50 opacity-50 cursor-not-allowed"
                    : "border-[#2a2a38] bg-[#12121a] hover:border-[#00ff88] cursor-pointer"
                )}
              >
                <div
                ...
                <p className="font-bold text-[#f0f0f5] text-sm">{team.name}</p>
                {isSelected && selectedByManager && (
                  <p className="text-xs text-[#00ff88] mt-1">{selectedByManager.name}</p>
                )}
              </button>
            )
          })}
        ...
        {state.teams.length > 0 && (
          <div className="mt-8 p-4 rounded-xl bg-[#12121a] border border-[#2a2a38]">
            <h3 className="text-sm font-bold text-[#888899] mb-3">Times Selecionados</h3>
              ...
              {state.teams.map((team) => {
                ...
                return (
                  <div key={team.id} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-[#f0f0f5]">{manager?.name}</span>
                  </div>
                )
              })}
            </div>
